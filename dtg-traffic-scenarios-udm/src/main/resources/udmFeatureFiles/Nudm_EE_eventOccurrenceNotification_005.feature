Feature: AMF Registration

#This scenario should be run after Nudm_EE_CreateEeSubscription_003.feature to trigger notifications to NEF.
#Notification will not sent in FT, because it needs a NEF server.
...
Scenario: Default callback handler initialization
  ...
  Given callback request path prefix /UeReachabilityReport
  Given action name is default-notify-ue-reachability
