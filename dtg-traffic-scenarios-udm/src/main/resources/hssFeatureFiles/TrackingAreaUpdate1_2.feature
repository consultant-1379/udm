...
Scenario: Update Location Request
  ...
  [
    ...
    { "name":"Supported-Features", "avps":[
            ...
            { "name":"Feature-List", "value":1114112 }
        ]
    }
  ]
  """
  ...
  Then we expect Diameter result DIAMETER_SUCCESS
  Then we expect Diameter response time less than 4000 milliseconds
  Then we expect Diameter response before action CLR_Verify

Scenario: wait for incoming Cancel-Location-Request from HSS
  Then we wait for Diameter Cancel-Location-Request for 4 seconds

Scenario: Delay
  Given action name is ULA_receiving
  Then Sleep for 1 seconds

Scenario: Any action
  Given action name is CLR_Verify
  Then Sleep for 1 seconds
