...
Scenario: prepare for incoming Delete-Subscriber-Data-Request from HSS
  Given Incoming Diameter Delete-Subscriber-Data-Request target type is EPC
  Given Incoming Diameter Delete-Subscriber-Data-Request application is S6a
  Given Incoming Diameter Delete-Subscriber-Data-Request key AVP is User-Name
  When we receive incoming Diameter Delete-Subscriber-Data-Request for <imsiprefix><<keyIndex>>
  Then save Incoming Diameter Delete-Subscriber-Data-Request AVP Session-Id value as sessionId
  Then we send Diameter answer for Delete-Subscriber-Data-Request with AVPs:
  """
    [
...
Scenario: UpdateEPCProfile
  ...
  Given SOAP request property ${Properties#mscId} equals <mscidprefix><<keyIndex>>
  Given SOAP request property ${Properties#imsi} equals <imsiprefix><<keyIndex>>
        ...
        <soap:Body>
			...
			<ldapAttributeValue>${Properties#imsi}</ldapAttributeValue>
                ...
                <modificationInformation typ:dN="EpsStaInfId=EpsStaInf,serv=EPS,mscId=${Properties#mscId},ou=multiSCs,dc=operator,dc=com" typ:operation="modify">
                    <affectedLdapAttribute typ:binary="false" typ:ldapAttributeName="EpsIndUeUsageType" typ:modification="remove">
                        <oldLdapAttributeValue>22</oldLdapAttributeValue>
...
Scenario: wait for incoming Delete-Subscriber-Data-Request from HSS
  Then we wait for Diameter Delete-Subscriber-Data-Request for 30 seconds
