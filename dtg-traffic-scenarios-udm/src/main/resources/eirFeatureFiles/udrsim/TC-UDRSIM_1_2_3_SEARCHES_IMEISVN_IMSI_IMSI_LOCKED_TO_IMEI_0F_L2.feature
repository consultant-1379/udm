Feature:  EIR data population for IMEISVN + IMSI (IMSI locked to IMEI) (1-3 Searches) Whitelisted
# This covers following scenario:
# - IMEI_CHECK_WHITELISTED_IMEISVN_IMSI_IMSI_LOCKED_TO_IMEI_0F
...
Scenario: Populate IMEISVN data
  ...
  [{
    ...
    "ldapAttributes": [
      ...
      {"name": "l2", "values": [{"ldapAttributeValueB":"AQ=="}]},
  ...
  [{
    ...
    "ldapAttributes": [
      {"name": "IMSI", "values": [{"ldapAttributeValueS":"<imsi>"}]},
	    {"name": "lockedImeiSvn", "values": [{"ldapAttributeValueS":"<imei>0F"}]},
      {
        "name": "objectClass",
        "values": [
          {"ldapAttributeValueS": "top"},
